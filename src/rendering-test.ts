...
const getSpriteXFromFrameId = (
  ...
  const animationSpeed = .08
  ;
  const animationBasis = gameState.time.stepID;
  const movingMultiplier = 2;
  const playerSpriteX = 0 + Math.floor((animationBasis * (animationSpeed * (gameState.player.moving ? movingMultiplier : 1))) % 4) * spriteFrameSize;
  return playerSpriteX;
};
...
const getSpriteXFromFrameIdSimple = (
  ...
  const position = Math.floor(gameState.time.stepID / staggerFrames) % 3;
...
switch (offset) {
  case 0:
    playerSpriteX = getSpriteXFromFrameIdSimple(ctx, canvas, gameState, spriteFrameSize);
    break;
  case 1:
    playerSpriteX = getSpriteXFromFrameId(ctx, canvas, gameState, spriteFrameSize);
    break;
  case 2:
    playerSpriteX = getSpriteXFromLastFrameTimeMS(ctx, canvas, gameState, spriteFrameSize);
    break;
  case 3:
    playerSpriteX = getSpriteXFromDelta(ctx, canvas, gameState, spriteFrameSize);
    break;
  default:
    playerSpriteX = playerSpriteX = getSpriteXFromFrameIdSimple(ctx, canvas, gameState, spriteFrameSize);
}
  ...
  const canvasY = gameState.player.y + (offset * (canvas.width / 50) * 10 / 2);

  drawSprite(ctx, canvas, playerSpritesheet, {
    canvasX,
    canvasY,
    canvasWidth: (canvas.width / 50) * 10,
    canvasHeight: (canvas.width / 50) * 10,
...
export const render = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, gameState: GameState) => {
  ...
  let offs = 0;
  drawPlayerSprite(ctx, canvas, gameState, offs++);
  drawPlayerSprite(ctx, canvas, gameState, offs++);
  drawPlayerSprite(ctx, canvas, gameState, offs++);
  drawPlayerSprite(ctx, canvas, gameState, offs++);
